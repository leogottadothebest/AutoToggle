...
struct MenuBarContentView: View {
    ...
    private var headerView: some View {
        HStack {
            ...
            .accessibilityLabel(Text(sleepPreventionManager.isPreventingSystemSleep ? "关闭防系统休眠" : "开启防系统休眠"))
            ...
            .accessibilityLabel(Text(menuBarManager.isPaused ? "恢复规则执行" : "暂停所有规则"))
            ...
            .buttonStyle(.borderless)
            .help("退出 AutoToggle")
            .accessibilityLabel(Text("退出 AutoToggle"))
        }
        .padding(.horizontal)
        .padding(.vertical, 10)
    }
                ...
                LazyVStack(spacing: 0) {
                    ForEach(upcomingTriggers) { item in
                        HStack(spacing: 10) {
                            ...
                            .controlSize(.mini)
                            .labelsHidden()
                            .accessibilityLabel(Text(item.appName))
                        }
                        .padding(.horizontal)
                        .padding(.vertical, 4)
                    }
